...
Notes: ${specialInst}`.trim();
      ...
      const formattedDate = new Date(`${y}-${m}-${d}T${pickupTime || '12:00'}:00`);
              ...
              <div className={styles.inputGroup}>
                ...
                <div style={{ position: "relative" }}>
                  ...
                  <input 
                    ...
                    onChange={(e) => {
                      if(e.target.value) {
                        setPickupTime(e.target.value); // always HH:MM (24h) natively
                      }
                    }}
                  />
                  <i className="fa-regular fa-clock" style={{ position: "absolute", right: "15px", top: "50%", transform: "translateY(-50%)", color: "var(--text-light)", pointerEvents: "none", zIndex: 5 }}></i>
                </div>
              </div>
