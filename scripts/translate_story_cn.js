...
async function main() {
  ...
  const targetFilePath = path.join(__dirname, '../src/components/StoryContentCn.js');

  console.log(`Reading English story from: ${storyFilePath}`);
  const englishCode = fs.readFileSync(storyFilePath, 'utf8');

  console.log("Translating story to Chinese using Gemini...");
  const prompt = `You are an expert bilingual translator specialized in English and Chinese localization.
...
Translate all visible user-facing text, chapter titles, image captions/alts, button labels, and paragraph contents into natural, elegant, upscale, and contextually accurate Simplified Chinese.
...
2. Translate only the literal string contents inside tags (e.g. <h2>Chapter 1: The Genesis of That Laundry Shop</h2> -> <h2>第一章：That Laundry Shop 的创立</h2>).
...
5. Rename the component function from \`StoryContent\` to \`StoryContentCn\` inside the code.
...
${englishCode}`;
  ...
  try {
    ...
    let ChineseCode = response.text.trim();
    
    // Strip markdown code block wrappers if the model still generated them
    if (ChineseCode.startsWith("```")) {
      ChineseCode = ChineseCode.replace(/^```[a-zA-Z]*\n/, "").replace(/\n```$/, "");
    }

    console.log(`Writing translated Chinese story to: ${targetFilePath}`);
    fs.writeFileSync(targetFilePath, ChineseCode, 'utf8');
